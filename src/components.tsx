...
export function YardageReferenceSheet({ project, rows }: { project: Project; rows: YardageRow[] }) {
  ...
  if (!row) return null;
  return <section className="yardage-reference"><strong>Concrete reference sheet</strong><div><span>State <b>{row.state || "—"}</b></span><span>Supplier <b>{row.concreteCompany || "—"}</b></span><span>Dimensions <b>{row.dimensions}</b></span><span>Thickness <b>{row.thickness} in</b></span>{row.secondaryDimensions && <><span>Secondary section <b>{row.secondaryDimensions} · {row.secondaryThickness} in</b></span><span>Secondary area <b>{row.secondaryAreaYardage.toFixed(2)} CY</b></span></>}<span>Footers <b>{row.footers}</b></span><span>Slab sq. ft. <b>{row.slabSquareFeet.toFixed(0)}</b></span><span>Slab CY <b>{row.slabYardage.toFixed(2)}</b></span><span>Footer CY <b>{row.footerYardage.toFixed(2)}</b></span><span>Total CY <b>{row.totalYardage.toFixed(2)}</b></span><span>Additional CY <b>{row.additionalConcreteYardage.toFixed(2)}</b></span><span>Waste / overage <b>{row.wasteOverageYardage.toFixed(2)}</b></span><span className="yardage-final">Final order <b>{row.finalOrderYardage.toFixed(2)} CY</b></span></div></section>;
}
