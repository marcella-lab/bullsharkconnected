...
function ClientProjects({ data }: { data: BootstrapPayload }) {
  return <><PageHeading eyebrow="My work" title="My projects" detail="Every job scope is organized beneath the project it belongs to." /><div className="project-stack">{data.projects.map((project) => <section className="project-card client-project" key={project.id}><header><div className="project-identity"><span className="project-code">{project.number}</span><h2>{project.name}</h2><p><MapPin size={14} /> {project.address}</p></div><div className="project-summary"><div><small>Stage</small><strong>{project.currentStage}</strong></div><div><small>Progress</small><strong>{project.progress}%</strong></div><div><small>Target</small><strong>{dateLabel(project.targetDate)}</strong></div></div></header><ProgressBar value={project.progress} /><div className="client-job-grid">{data.jobs.filter((job) => job.projectId === project.id).map((job) => <article key={job.id}><div><StatusPill tone={job.status === "in_progress" ? "cyan" : job.status === "scheduled" ? "orange" : "neutral"}>{job.status.replaceAll("_", " ")}</StatusPill><small>{job.number}</small></div><h3>{job.title}</h3><p>{job.scope}</p><dl><div><dt>Stage</dt><dd>{job.stage}</dd></div><div><dt>Progress</dt><dd>{job.progress}%</dd></div><div><dt>Scheduled</dt><dd>{job.scheduleStart ? `${dateLabel(job.scheduleStart)} – ${dateLabel(job.scheduleEnd)}` : "Pending"}</dd></div></dl></article>)}</div></section>)}</div></>;
}
...
export function SubcontractorPages({ data, view, mutate }: { data: BootstrapPayload; view: string; mutate: Mutation }) {
  ...
  if (view === "jobs") return <SubJobs data={data} jobs={assigned} />;
  if (view === "schedule") return <SubSchedule data={data} jobs={scheduled} />;
  if (view === "contracts") return <SubContracts data={data} />;
...
function SubContracts({ data }: { data: BootstrapPayload }) {
  return <><PageHeading eyebrow="Agreements" title="My contracts" detail="Download agreements generated for your assigned work and track signature status." />{data.contracts.length === 0 ? <EmptyState title="No contracts available" detail="Your agreements will appear here when BullShark assigns work." /> : <div className="contract-grid">{data.contracts.map((contract) => { const job = data.jobs.find((item) => item.id === contract.jobId); return <article className="contract-card" key={contract.id}><header><span className="document-icon"><FileSignature /></span><StatusPill tone={contract.status === "sent" || contract.status === "ready" ? "cyan" : "neutral"}>{contract.status}</StatusPill></header><small>{contract.contractNumber}</small><h2>{job?.title}</h2><p>{currency.format(contract.price)}</p><footer><button className="button button-small button-dark" onClick={() => void api.downloadContract(contract.id, contract.contractNumber, "subcontractor")}><Download size={15} /> Download agreement</button></footer></article>; })}</div>}</>;
}
