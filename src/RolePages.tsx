...
import { api } from "./api";
import { isPhotoFile } from "./fileUtils";
      ...
      <section className="project-stack">{data.projects.map((project) => {
        ...
        return <section className="project-card client-project" key={project.id}>
          <header>
            ...
            <div className="project-summary client-project-summary">
              <div className="client-progress-wheel" style={{ background: `conic-gradient(#0a9db7 ${project.progress * 3.6}deg, #e4eef1 0deg)` }}><span><b>{project.progress}%</b><small>complete</small></span></div>
              <div><small>Shared photos</small><strong>{photoCount}</strong></div>
              <div><small>Shared files</small><strong>{sharedFiles.length - photoCount}</strong></div>
...
function SubSchedule({ data, jobs, onOpenProject }: { data: BootstrapPayload; jobs: Job[]; onOpenProject?: (project: import("./types").Project) => void }) {
  return <><PageHeading eyebrow="Field commitments" title="My schedule" detail="BullShark-published dates for the work assigned to your company." />{jobs.length === 0 ? <EmptyState title="No scheduled work" detail="Assigned dates will appear here when BullShark publishes the schedule." /> : <div className="sub-calendar">{jobs.map((job) => { const project = jobProject(data, job); return <article key={job.id}><div className="calendar-range"><span><small>START</small><strong>{dateLabel(job.scheduleStart)}</strong></span><ArrowRight /><span><small>FINISH</small><strong>{dateLabel(job.scheduleEnd)}</strong></span></div><div><small>{job.number}</small><h2>{job.title}</h2><p>{project?.name} · {job.location}</p></div><button className="button button-small" disabled={!project} onClick={() => project && onOpenProject?.(project)}>Open project</button><StatusPill tone="cyan">{job.status.replaceAll("_", " ")}</StatusPill></article>; })}</div>}</>;
}
