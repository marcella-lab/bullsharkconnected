...
async function migrate(data: PortalData) {
  ...
  for (const row of data.yardageRows || []) {
    ...
    if (row.secondaryDimensions === undefined || row.secondarySectionType === undefined || row.secondaryAreaYardage === undefined || row.thirdDimensions === undefined || row.noPourDimensions === undefined || row.wastePercent === undefined) {
      const subtotal = (row.totalYardage || 0) + (row.additionalConcreteYardage || 0);
      Object.assign(row, { secondaryDimensions: row.secondaryDimensions || "", secondaryThickness: row.secondaryThickness || 0, secondarySectionType: row.secondarySectionType || "inside", secondarySquareFeet: row.secondarySquareFeet || 0, secondaryAreaYardage: row.secondaryAreaYardage || 0, thirdDimensions: row.thirdDimensions || "", thirdThickness: row.thirdThickness || 0, thirdSectionType: row.thirdSectionType || "inside", thirdSquareFeet: row.thirdSquareFeet || 0, thirdAreaYardage: row.thirdAreaYardage || 0, thirdFooters: row.thirdFooters || "", thirdFooterYardage: row.thirdFooterYardage || 0, thirdAdditionalConcreteYardage: row.thirdAdditionalConcreteYardage || 0, noPourDimensions: row.noPourDimensions || "", noPourThickness: row.noPourThickness || 0, noPourSquareFeet: row.noPourSquareFeet || 0, netMainSquareFeet: row.netMainSquareFeet ?? row.slabSquareFeet, totalCoveredSlabSquareFeet: row.totalCoveredSlabSquareFeet ?? row.slabSquareFeet, mainSlabYardage: row.mainSlabYardage ?? row.slabYardage, keepFooterAroundNoPour: row.keepFooterAroundNoPour ?? true, additionalFooterDepth: row.additionalFooterDepth ?? Math.max((row.footerDepth || 0) - row.thickness, 0), wastePercent: subtotal > 0 ? ((row.wasteOverageYardage || 0) / subtotal) * 100 : 0, recommendedOrderYardage: row.recommendedOrderYardage || Math.ceil(row.finalOrderYardage || 0) }); changed = true;
    }
  }
