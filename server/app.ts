...
export function createApp(store: DataStore, esign: EsignService = new ConfiguredEsignService()) {
  ...
  app.patch("/api/projects/:projectId/client-contact", requireRole("admin"), asyncRoute(async (req, res) => { const input = z.object({ name: z.string().trim().min(2).max(160), email: z.string().trim().email().max(200), phone: z.string().trim().max(60).optional(), contractCost: z.coerce.number().nonnegative() }).parse(req.body); const result = await store.update((data) => { const project = data.projects.find((item) => item.id === req.params.projectId); if (!project) throw Object.assign(new Error("Project not found."), { status: 404 }); const client = data.clients.find((item) => item.id === project.clientId); if (!client) throw Object.assign(new Error("Client not found."), { status: 404 }); client.name = input.name; client.email = input.email; client.phone = input.phone || undefined; project.budget = input.contractCost; data.projects.filter((item) => item.clientId === client.id).forEach((item) => { item.clientName = input.name; }); data.jobs.filter((item) => item.clientId === client.id).forEach((item) => { item.clientName = input.name; }); // Contact records and portal-access users are intentionally separate. Editing a
    // project's client contact must never rename, re-email, or grant access to a user.
    audit(data, "Client contact updated", `${project.number}: ${input.name} contact and contract cost updated.`); return { project, client, phone: client.phone }; }); res.json(result); }));
