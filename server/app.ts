...
export function createApp(store: DataStore, esign: EsignService = new ConfiguredEsignService()) {
  ...
  // Job progress describes a specific scope. Project progress is a separate,
  // administrator-controlled overall percentage and must not jump to 100%
  // merely because one job (for example, Foundation) is complete.
  const refreshProject = (data: PortalData, projectId: string) => { const project = data.projects.find((candidate) => candidate.id === projectId); if (!project) return; project.status = project.progress >= 100 ? "complete" : "active"; };
