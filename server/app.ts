...
export function createApp(store: DataStore, esign: EsignService = new ConfiguredEsignService()) {
  ...
  app.get("/api/files/:fileId/preview", asyncRoute(async (req, res) => { const data = await store.read(); const file = data.files!.find((item) => item.id === req.params.fileId); const user = userById(data, req.viewer.id); if (!file || !file.mimeType.startsWith("image/") || !canProject(data, user, file.projectId) || (req.viewer.role === "subcontractor" && file.visibility !== "project_access" && !file.jobIds.some((job) => user?.jobIds.includes(job))) || (req.viewer.role === "client" && !["client", "client_and_assigned_subcontractor", "project_access"].includes(file.visibility))) return res.status(403).json({ message: "You do not have access to this image." }); res.type(file.mimeType); res.sendFile(file.path); }));
